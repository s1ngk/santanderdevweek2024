const routes = {
  championsall: "http://localhost:8080/champions",
  championsask: "http://localhost:8080/champions/{id}/ask",
}

const apiService = {
  async getChampions () {
    const route = routes.championsall;
...
async function renderChampions(){
  ...
  const elements = championsData.map(
    (character) =>        `<div class="timeline-carousel__item" onclick="onChangeChampionSelected(${character.id}, '${character.imageUrl}')">
...
async function getRandomQuote(){
  const quotes = [
    ...
    "Ainda estou tentando entender como essa giringonça funciona...",
